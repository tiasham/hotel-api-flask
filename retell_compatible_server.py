...
server = RetellCompatibleServer()

@app.route('/')
def home():
    """Root endpoint"""
    return jsonify({
        'message': 'Retell-Compatible MCP Server',
        'version': '1.0.0',
        'status': 'running',
        'endpoints': {
            'health': '/health',
...
def health():
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'Retell-Compatible MCP Server is running',
        'timestamp': datetime.now().isoformat()
    })

@app.route('/tools', methods=['GET'])
def get_tools():
    """Get available tools - Retell-compatible format"""
    return jsonify({
        "tools": [
            {
                "name": "searchHotels",
...
def execute_tool():
    """Execute a tool - Retell-compatible format"""
    try:
        data = request.json
