#!/usr/bin/env python3
"""
Retell-Specific MCP Server - Designed for Retell's exact requirements
"""
...
def home():
    """Root endpoint"""
    return jsonify({
        'message': 'Retell-Specific MCP Server',
        'version': '1.0.0',
        'status': 'running',
        'endpoints': {
            'health': '/health',
...
def health():
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'Retell-Specific MCP Server is running',
        'timestamp': datetime.now().isoformat()
    })

@app.route('/tools', methods=['GET'])
def get_tools():
    """Get available tools - Retell-specific format"""
    return jsonify({
        "tools": [
            {
                "name": "searchHotels",
...
def execute_tool():
    """Execute a tool - Retell-specific format"""
    try:
        data = request.json
